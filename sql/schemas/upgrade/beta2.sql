...
PROMPT Upgrading SCHEMAS to beta2...
...
-- SECTION: LIVESQL-753
STORE SET system_variables_temporary_store.sql
...
-- SECTION: LIVESQL-823
-- NOTE: Update tablespace for all existing schemas
BEGIN
    FOR L_SCHEMA IN ( SELECT NAME FROM LIVE_SQL.SCHEMAS ) LOOP
        EXECUTE IMMEDIATE 'ALTER USER "' || DBMS_ASSERT.SCHEMA_NAME( L_SCHEMA.NAME ) || '" DEFAULT TABLESPACE USERS QUOTA 10M ON USERS';
    END LOOP;
END;
/
-- !SECTION: LIVESQL-823
...
-- SECTION: LIVESQL-789
-- NOTE: Update grants for all existing schemas
BEGIN
    FOR L_SCHEMA IN ( SELECT NAME, IS_READ_ONLY FROM LIVE_SQL.SCHEMAS ) LOOP
        IF L_SCHEMA.IS_READ_ONLY = 'Y' THEN
            EXECUTE IMMEDIATE 'GRANT LIVE_SQL_READ_ONLY_USER TO "' || DBMS_ASSERT.SCHEMA_NAME( L_SCHEMA.NAME ) || '"';
        ELSE
            IF DBMS_DB_VERSION.VER_LE_23 THEN
                EXECUTE IMMEDIATE 'REVOKE CREATE DOMAIN FROM "' || DBMS_ASSERT.SCHEMA_NAME( L_SCHEMA.NAME ) || '"';
            END IF;
            EXECUTE IMMEDIATE 'GRANT LIVE_SQL_READ_WRITE_USER TO "' || DBMS_ASSERT.SCHEMA_NAME( L_SCHEMA.NAME ) || '"';
        END IF;
    END LOOP;
END;
/
...
FROM
    ALL_TAB_PRIVS_RECD
WHERE
    GRANTEE = ''LIVE_SQL_SAMPLE_SCHEMAS_USER'''
    );
    -- !SECTION: LIVESQL-458
...
PROMPT Upgrade on SCHEMAS to beta2 complete
