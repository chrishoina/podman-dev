PROMPT Upgrading SCHEMAS to r35...
...
BEGIN
    -- Added validation to avoid granting privileges on DB versions without MLE feature (available from 23ai onwards)
    -- DB 19c doesn't support MLE
    IF DBMS_DB_VERSION.VERSION >= 23 THEN
        EXECUTE IMMEDIATE q'[CREATE OR REPLACE PROCEDURE GRANT_EXECUTE_ON_JAVASCRIPT_TO_SCHEMA( P_SCHEMA_NAME VARCHAR2 )
        AUTHID DEFINER
        AS
        BEGIN
            EXECUTE IMMEDIATE 'GRANT EXECUTE ON JAVASCRIPT TO ' || DBMS_ASSERT.ENQUOTE_NAME( P_SCHEMA_NAME );
        END;]';
        EXECUTE IMMEDIATE 'GRANT EXECUTE ON GRANT_EXECUTE_ON_JAVASCRIPT_TO_SCHEMA TO LIVE_SQL';
    END IF;
END;
/
-- !SECTION: LIVESQL-840
    ...
    IF DBMS_DB_VERSION.VERSION >= 23 THEN
        FOR L_SCHEMA IN ( SELECT NAME, IS_READ_ONLY FROM LIVE_SQL.SCHEMAS ) LOOP
            IF L_SCHEMA.IS_READ_ONLY !='Y' THEN
                    SYS.GRANT_EXECUTE_ON_JAVASCRIPT_TO_SCHEMA( L_SCHEMA.NAME );
            END IF;
        END LOOP;
    END IF;
END;
...
-- Update the SCHEMA_MANAGEMENT package to add the "execute on javascript" privilege directly to new created schemas
...
PROMPT Upgrade on SCHEMAS to r35 complete
